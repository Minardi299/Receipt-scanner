...
def process_img(img: np.ndarray) -> np.ndarray:
    ...
    blackwhite_img = cv2.adaptiveThreshold(grey_img, 200, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 15, 10)#cv2.threshold(grey_img, 130, 230, cv2.THRESH_BINARY)
...
def img_to_stringpdf(img: np.ndarray) -> str:
    ...
    grey_img=cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
    #tweak the 150 and 230 value for better result
    thresh,blackwhite_img = cv2.adaptiveThreshold(grey_img, 200, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 15, 10)#cv2.threshold(grey_img, 130, 230, cv2.THRESH_BINARY)
...
if __name__ == '__main__':
    ...
    img=cv2.imread('TestingImgs/IMG_20231219_1836282.jpg')

    fixed = process_img(img)

    cv2.imwrite("processed/OGRemoveNoise.jpg", fixed)
    text =pytesseract.image_to_string(fixed)
    destination_path = f"Result/OG.txt"
